...
case "$LAYOUT" in
	...
	rootful)
		...
		;;
	*)
		echo "error: IOS_LAYOUT must be auto, rootless, or rootful" >&2
		exit 1
		;;
esac
if [ ! -x "$LAUNCHCTL" ]; then
...
install -m 0755 "$OUT_DIR/ios-arm64/zola-$VERSION-ios-arm64" "$PKG_ROOT$ZOLA_BIN"
